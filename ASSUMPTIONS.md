...
| 2026-08-05 | **Default model slug `anthropic/claude-sonnet-4.5`**, overridable via `MODEL_DRAFTER` / `MODEL_CRITIC`. | Cost/quality balance for daily drafting + judging; kept out of code so it can be pinned per-deploy. | Pin a specific Claude model, or if the slug is unavailable on your Gateway. |
